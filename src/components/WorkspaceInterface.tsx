    ...
    <div className="flex h-screen bg-gray-900 text-gray-100 font-sans overflow-hidden"> {/* Prevent body scroll */}
      ...
      <div className={`bg-gray-800 border-l border-gray-700 flex flex-col transition-all duration-300 ease-in-out shrink-0 ${rightPanelVisible && activeModule ? 'w-80' : 'w-0'} overflow-hidden`}>
        {rightPanelVisible && activeModule && selectedItemForDetails && (
         ...
         {/* Show a prompt if panel is open but no item is selected */}
         {rightPanelVisible && activeModule && !selectedItemForDetails && (
            <div className="flex-1 flex flex-col items-center justify-center p-4 text-center">
                 <Info size={32} className="text-gray-600 mb-3" />
                <p className="text-sm text-gray-500">Select an item from the list to see its details here.</p>
            </div>
         )}
      </div>
      ...
      {!rightPanelVisible && activeModule && (
        <button
          onClick={() => setRightPanelVisible(true)}
          className="fixed right-0 top-1/2 transform -translate-y-1/2 bg-gray-700 border border-gray-600 border-r-0 shadow-lg rounded-l-lg p-2.5 text-gray-300 hover:text-white hover:bg-gray-600 z-30"
